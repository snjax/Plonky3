...
pub fn decompose_and_flatten<C: Config>(
    quotient_poly: Vec<C::Challenge>,
    shift: C::Challenge,
    log_chunks: usize,
) -> RowMajorMatrix<C::Val> {
    let chunks: Vec<Vec<C::Challenge>> = decompose(quotient_poly, shift, log_chunks);
    let degree = chunks[0].len();
    let quotient_chunks_flattened: Vec<C::Val> = (0..degree)
        .into_par_iter()
        .flat_map_iter(|row| {
            chunks
                .iter()
        ...
        .collect();
    let challenge_ext_degree = <C::Challenge as AbstractExtensionField<C::Val>>::D;
    RowMajorMatrix::new(
